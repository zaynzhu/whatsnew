...
- 同一 `MediaItem` 在同一榜单中出现多个季或版本时，必须用稳定 `rankingEntryKey` 分开保存，并用 `rankingEntryLabel` 展示季或版本；不得为了作品归并而覆盖合法榜单席位
...
- Netflix 当前榜单优先读取 Tudum 四个页面路由；只有四类同周且每类完整包含 1–10 名时才采用，否则整批回退官方 XLSX。XLSX 距当前时间超过 14 天时必须拒绝降级同步，不能覆盖更新的现有快照。页面发行年份允许以 `YYYY` 精度写入匹配字段，严格 TMDb 身份命中后必须升级为真实首播日；横版 `sdpArt` / `storyArt` 不得写入竖版海报字段。完整榜单同步必须停用已下榜的 Netflix 来源引用
...
- Hulu 标题中的 `Season`、`Series`、`Film` 与 `Special Premiere` 必须清理为作品标题并保留原文别名；首页优先平台首发，普通排期次之，旧目录新增不获得时效加成，但日历仍完整保留
- 首页今日与本周片单必须先按作品去重再截取，不能让同日多集或多来源排期重复占位；日历和详情页继续保留完整排期
- 首页事件流的 `source_failed` 必须按同一来源和 `eventAt` 的 UTC 日期只展示最新一条；原始 `ChangeEvent` 历史继续保留，不能为展示去重删除运行记录
...
- 缺图与低清替换队列必须使用设置页的内容关注权重和 `heatScore` 共同排序；新闻、晨间节目和体育等低关注内容不得仅因来源刚同步而抢占剧情、动画或高热待播作品的补图名额
- 缺图补全必须在图片健康接口中区分尚未尝试、3 天冷却中和可以重试；最近尝试时间只表示严格查询已执行，不表示已经找到可靠匹配
- TMDb 网络、限流、认证或服务端错误不得启动作品级 3 天冷却；只有完成查询后的无匹配结果和已确认不存在的直接 TMDb 身份才算一次有效尝试
- 图片健康接口和设置页必须分别统计缺图补全与低清替换任务，不能把两类任务的未尝试、冷却中和可重试数量混为一组
- 图片健康接口与设置页的“高优先级”样本必须复用真实补图队列的关注权重与 Heat 算法，并显示内容类别和优先级分数，不得继续按旧 Heat 顺序误导运维判断
