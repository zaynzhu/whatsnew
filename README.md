...
- 豆瓣 adapter 会把官方图域的 `s_ratio_poster` 规范为同一素材的 `l_ratio_poster` 后入库；已有小图只在同一稳定豆瓣来源身份和相同素材路径下升级。官方 `movie*.jpg` / `tv*.jpg` 通用占位图按缺图处理，同时从同一 subject 详情保留原名、别名和国家信息供严格身份查询，避免把占位素材或翻译猜测当成真实海报
...
- 同一作品的不同季或版本使用独立榜单条目身份保存名次，热度页仍按作品聚合并显示各条目的季标题，不会互相覆盖
...
Netflix 来源优先读取 Tudum 官方当前周页面，只在四类榜单周次一致、每类完整包含 1–10 名时采用：英语电影、非英语电影、英语剧集和非英语剧集，共 40 条当前信号。页面不可用或任一榜单不完整时，整批回退官方全球全周 XLSX，不混用不同周次；XLSX 距当前时间超过 14 天时拒绝降级同步，避免覆盖更新的现有快照。

- 每日 `09:15`（`Asia/Shanghai`）检查一次，启动同步仍遵循 `SYNC_ON_START`
- 页面元数据会补充发行年份与简介；严格 TMDb 身份命中后会把低精度 `YYYY` 升级为真实首播日，Tudum 横版素材不写入竖版海报字段
- 同一周重复同步按来源身份和周次保持幂等；完整快照会停用已下榜来源引用，但保留历史影视记录
- 下载沿用统一代理设置、页面 30 秒超时、XLSX 10 秒超时和来源级 2 秒限频
...
- 首页、发现、日历、热度榜和前瞻只展示仍有启用来源的当前作品；失效来源历史继续保留，可通过原作品 ID 查看详情和历史信号
- 作品状态按精确首发日期做一致性约束：未来首发不能写成已上映，首发日已到后不能继续写成即将上线或未知；没有精确日期时保持未知，平台预约、上架和目录新增仍由带来源的 `Release` 表达
...
- 同步额外读取电影和剧集 `sortby=hot` 独立榜单各前 20，保存为 `douban_upcoming_hot`；前瞻时间线保持日期排序，每个日期内按热门名次优先排列并高亮命中作品
- `/preview` 不做列表截断，完整返回当前仍有效的已定档和待定档电影、剧集；页面每个日期自动换行展示全部海报，不再通过隐藏的横向滚动截断可见片单
