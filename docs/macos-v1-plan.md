...
状态：第一版基线已完成，进入持续增量完善
日期：2026-08-15

当前实现已经覆盖 M0–M6，并生成仅供本人使用的 `dist/WhatsNew.app`。后续开发继续沿用本计划的产品、安全和 API 边界，以真实运行验收发现的缺口为单位做原子改进；GitHub Actions 只检查源码构建与测试，不上传应用、不创建 Release。
