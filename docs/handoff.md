...
- Tencent Video passed a fresh isolated sandbox sync on 2026-07-14: 208 active source identities, 208 undated upcoming releases, 208 posters and 27 reservation signals; 196 works were new and 12 matched existing identities. After visual acceptance, the source was enabled and the same adapter was synced into the main database with exactly the same 196/12 identity split. Two bounded verification batches measured all 208 Tencent posters as healthy and adequate, with minimum dimensions of `350×490`. The missing dates are an explicit upstream boundary: the channel filter proves “即将上线”, but `publish_date` is work metadata and must not be treated as a Tencent availability date.
...
- Placeholder-detail enrichment now retains Douban's official `original_title`, `aka` and `countries`. An isolated 2026-07-14 sandbox sync updated identity metadata for 16 of the 29 missing-poster works and gave 15 exact aliases; a strict sandbox TMDb retry scanned those 15 with zero false merges, conflicts or failures, while no official poster was yet available. The same adapter was then promoted to the main database: the latest `popularity` and `upcoming` runs completed successfully with 20 and 207 items, and the 29 active Douban works still missing artwork now include 15 with official aliases for future strict retries.
- The same validation exposed and fixed a maintenance-script initialization hazard: `config/db.ts` now loads the selected environment file before Prisma is created, so `SETTINGS_ENV_PATH=.env.china-sandbox` cannot silently fall back to the main database.
...
- A 2026-07-14 network recovery pass resynced the three degraded enabled sources in sequence: TVmaze returned 224 items, TMDb 71, Trakt popularity 199 and Trakt calendar 208. After Tencent promotion, the final source-health matrix reports all 11 runnable scopes passed, zero degraded/failed/stale scopes and 13 intentionally blocked coverage rows.
...
- The next poster phase-two pass on 2026-07-14 processed all 12 retry-eligible missing works. Eight completed as strict unmatched lookups; four initial TMDb transport failures remained retryable and completed as unmatched on one bounded retry. The later corrected Hulu sync reset 11 cleaned-title candidates: strict enrichment added verified `500×750` TMDb artwork for `They Fight` and `Disney Celebrates America`, with nine unmatched and no conflict or failure. The final audit reports 91.3% coverage, zero unverified/degraded/broken images, 110 cooling missing works and three cooling undersized low-attention news/sports images. The next missing-artwork window starts at 2026-07-16 12:52 Asia/Shanghai.
- `MediaPoster` now distinguishes upcoming artwork that has not been published, ordinary missing artwork and a poster that failed through both proxy and direct delivery. Real desktop and 390-pixel mobile checks on `NCIS: New York` kept the fallback inside fixed poster dimensions with no overflow, while the newly enriched `Crystal Lake` poster still rendered normally.
- Hulu schedule parsing now strips `Season`, `Series`, `Film` and `Special Premiere` qualifiers into retained aliases and classifies those rows as platform premieres instead of catalog additions. Dashboard ranking gives platform premieres a timing boost, removes that boost from catalog additions and deduplicates the today/week shelves by work. A main-database read-only request returned 12 today and 24 week works with zero duplicate media IDs. The corrected main sync then completed with 176 items: 19 platform premieres, 157 catalog additions and zero remaining premiere-qualified display titles.
...
1. Continue poster-system phase two when the next window opens on 2026-07-16: recheck the 110 cooling missing titles and three undersized titles without bypassing cooldown. Four additional Trakt titles remain immediately retryable because two bounded TMDb attempts ended in transport failure and correctly did not start a work cooldown. Keep the content-attention queue order and existing strict TMDb, IMDb and TheTVDB identity matching; source-generic placeholders must stay classified as missing until a trustworthy replacement exists.
