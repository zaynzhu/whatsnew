...
- Scheduled quality maintenance first repairs exact-date lifecycle contradictions, then reconciles conflict-free duplicate TMDb identities, conservative unique-title records with exactly one external identity anchor, and source-only records with independently corroborated exact dates; startup and daily runs also remove strict inactive platform orphans.
