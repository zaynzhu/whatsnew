...
- Poster maintenance after the disabled-source cleanup on 2026-07-14 reports 1,255 active works, 1,130 healthy posters, zero unverified/degraded/broken images, 125 missing and 4 undersized posters. A live OMDb check exposed both failure modes now guarded in code: one exact IMDb result returned an unavailable Amazon URL, and another returned a 300×169 horizontal image. Both records were restored to their verified TVmaze artwork without changing identity data.
