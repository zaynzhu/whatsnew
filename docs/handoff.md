...
- Tencent Video passed a fresh isolated sandbox sync on 2026-07-14: 208 active source identities, 208 undated upcoming releases, 208 posters and 27 reservation signals; 196 works were new and 12 matched existing identities, while the main database stayed unchanged. The missing dates are an explicit upstream boundary: the channel filter proves “即将上线”, but `publish_date` is work metadata and must not be treated as a Tencent availability date.
...
- A 2026-07-14 network recovery pass resynced the three degraded enabled sources in sequence: TVmaze returned 224 items, TMDb 71, Trakt popularity 199 and Trakt calendar 208. The final source-health matrix reports all 10 runnable scopes passed, zero degraded/failed/stale scopes and 14 intentionally blocked coverage rows.
