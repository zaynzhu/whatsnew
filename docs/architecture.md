...
5. Releases and popularity signals are upserted with original source attribution. Popularity snapshot identity also includes `rankingScope`, so Trakt movie/show charts, the four Netflix Top 10 categories and Bilibili bangumi/guochuang/documentary charts retain independent ranks.
...
2. `enrichMissingPosters()` selects missing, broken and undersized titles with at least one active source reference, then ranks the full eligible set by 70% configurable content-attention weight and 30% `heatScore`, using update time only as a tie-breaker. This keeps scripted, animation and genuinely hot upcoming work ahead of low-attention news or sports rows that happen to sync more recently. Inactive history-only records remain stored but stay outside poster health and maintenance queues.
...
5. If the chosen TMDb identity already belongs to a safe same-title record in the same movie/series work kind, source refs, releases, popularity signals and events move transactionally to the canonical record. Generic and specialized content types may merge only after this strict confirmation; animation, documentary, variety and short-form classifications take precedence and every external ID is retained. Unsafe identity conflicts, movie/series crossings and unresolved ambiguity are skipped.
6. Successful TMDb matches fill the poster URL and missing baseline metadata without overwriting existing values. Every TMDb poster candidate is downloaded before persistence and accepted only when it is currently available, at least 300×400 and portrait-oriented; a transport failure leaves the item retryable without replacing its current image. When TMDb has no usable image, an existing IMDb ID may use OMDb and an existing `tvdbId` may use the free TheTVDB detail endpoint as poster-only fallbacks. Neither fallback searches by title; the returned external ID and image checks must pass, so dead URLs, mismatched identities and horizontal artwork cannot replace an existing image.
7. Every completed unmatched attempt writes `posterLookupAttemptedAt`; unsuccessful records become eligible again after 3 days so new and upcoming titles can pick up newly published artwork without blocking untouched titles.
...
10. `prunePosterCache()` bounds original poster bytes at 2 GB and `prunePosterVariantCache()` bounds regenerable variants at 512 MB. Startup sync and daily maintenance remove corrupt and stale partial files from both stores, then evict each store's oldest complete entries to 90% when over capacity. Files newer than one hour are protected from partial-write cleanup.
11. The frontend falls back to the original URL only when the proxy request fails. If neither URL can render, `MediaPoster` distinguishes a not-yet-published upcoming poster from ordinary missing artwork and a two-path delivery failure without changing the poster slot dimensions.
...
15. Newly accepted TMDb, OMDb and TheTVDB enrichment images are stored as healthy with their measured dimensions because they already passed the shared image service. Hourly maintenance still verifies up to 20 high-heat eligible source-provided posters from currently active source records; startup sync and daily maintenance verify up to 100. Measurements are stored as `posterWidth` / `posterHeight`; widths below 300 or heights below 400 become `posterQuality=undersized`, independently of `posterStatus` availability. Degraded items wait one day and broken items seven days before retry.
16. Missing, broken and undersized posters enter the strict enrichment queue. TMDb remains the primary metadata source; exact IMDb identities may additionally use validated OMDb artwork, while an existing `tvdbId` may use artwork from the matching free TheTVDB detail record. Failed fallback artwork never overwrites the current poster, and transport failures remain immediately retryable. Unmatched records retry only after the three-day enrichment cooldown. `/api/poster-health` derives `not_attempted`, `cooldown` and `retry_eligible` lookup states from `posterLookupAttemptedAt` without treating an attempted lookup as a successful match; its high-priority samples use the same attention/Heat score as the worker and expose `attentionCategory` plus `priorityScore` for the Settings UI.
...
`completeMediaSources` marks a source sync as a complete catalog snapshot. References absent from the next complete snapshot become inactive. After scheduled syncs, records sharing any stable TMDb, TVmaze, IMDb, Trakt or TheTVDB identity are reconciled within the same movie/series work kind when the full connected component has no external-ID conflict. Canonical classification and poster choice are independent: specialized content types remain canonical, while artwork is chosen by availability status, quality state and then measured pixel area, with its health metadata moved as one unit. A second conservative pass merges source-only records only when normalized title and media type match, release years and trustworthy languages are compatible, and exactly one external-identity anchor exists; ambiguous titles remain separate. A third pass handles records with no external IDs only when at least two independent source families agree on normalized title, media type and the exact first-release date. Startup and daily maintenance additionally remove platform-only records only when every source reference is inactive and the work has neither releases nor popularity signals.
