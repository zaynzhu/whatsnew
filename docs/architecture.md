...
6. Successful TMDb matches fill the poster URL and missing baseline metadata without overwriting existing values. Every TMDb poster candidate is downloaded before persistence and accepted only when it is currently available, at least 300×400 and portrait-oriented; a transport failure leaves the item retryable without replacing its current image. When an item already has an IMDb ID, OMDb may provide a poster-only fallback through that exact ID; title search is never used and the same image checks apply, so dead Amazon URLs and horizontal artwork cannot replace an existing image.
