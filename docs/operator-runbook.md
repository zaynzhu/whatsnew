...
`reconcile:duplicate-identities` reports `tmdbIdentity`, `uniqueTitle` and `sharedDateTitle` separately. `tmdbIdentity` groups the same TMDb ID within a movie or series work kind, allowing a generic series record to merge into an animation, documentary, variety or short-form classification while keeping movie and series namespaces separate. `uniqueTitle` applies only when one external identity is uniquely anchored; `sharedDateTitle` requires at least two independent sources to agree on exact title, type and first-release date. `ambiguous` entries are never merged automatically. Always inspect the dry-run samples before using `--apply` on a new dataset.
